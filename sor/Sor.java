...
public class Sor {
	public static void main(String[] args) {
		...
		double omega = 1.1;
		double[] XO = {0, 0, 0};
		double[] rez = sor(koeficienti, tol, n0, omega, XO);
		for (int i = 0; i < XO.length; i++) {
			System.out.println("x" + (i + 1) + " = " + rez[i]);
		}
	}

	public static double[] sor(double[][] koeficienti, double tol, int n0, double omega, double[] XO) {
		int k = 1;
		...
		try {
			while (k <= n0) {
				for (int i = 0; i < XO.length; i++) {
					...
					rez[i] = (1 - omega) * XO[i]
							+ (omega * (-sum1 - sum2 + koeficienti[i][XO.length])) / koeficienti[i][i];
